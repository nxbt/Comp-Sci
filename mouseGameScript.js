...
function GameState() { //handles game states, which determin what will happen in game
  ...
  this.changeState = function(s) {//changes current state
    clearDisplay();
    switch(s) {
      case 0:
        state = new MenuState();
...
function MenuState() {//the main menu
  ...
  var button =[new Button(1), new Button(2)];
...
function ScoreState() {//displays the local score and high score
  ...
  var button = [new Button(1), new Button(0)];
...
function TutorialState() {
  ...
  text.id = "text";
  text.innerHTML = "Hello, and welcome to <i>You're the Mouse: The Game</i>! In this game, You must make sure that your mouse cursor doesn't touch anything white. If it does, it's game over! The longer you survive, the higher your score.<br/><br/>There are two types of obstacles: lasers, and the ball. lasers will just moves from one side of the screen to the other, but the ball will follow your mouse, so you'll have to always be on your guard.<br/><br/>Hover over the circle to exit the tutorial.";
  ...
  this.update = function() {
    if(button[0].update() === 0) gs.changeState(0);
  };
}
...
function gameOver() { //ends the game and exits to the score screen
  overlay.effect(1);
  gs.changeState(2);
}

function setCookie(cname, cvalue, exdays) { //creates or changes a cookie | made by w3schools: http://www.w3schools.com/js/js_cookies.asp
    var d = new Date();
...
function getCookie(cname) { //finds a cookie and returns the value | made by w3schools: http://www.w3schools.com/js/js_cookies.asp
    var name = cname + "=";
